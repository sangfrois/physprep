...
def sqi_cardiac(signal_cardiac, info, data_type='ECG', sampling_rate=10000):
    ...
    summary["Mean_NN_intervals"] = np.round(np.mean(info[f'{data_type}_clean_rr_systole']),4)
    summary["Median_NN_intervals"] = np.round(np.median(info[f'{data_type}_clean_rr_systole']),4)
    summary["SD_NN_intervals"] = np.round(np.std(info[f'{data_type}_clean_rr_systole'], ddof=1),4)
    # Quality indices on heart rate
    summary["Mean_HR"] = metrics_hr_sqi(info[f'{data_type}_clean_rr_systole'], metric='mean')
    summary["Median_HR"] = metrics_hr_sqi(info[f'{data_type}_clean_rr_systole'], metric='median')
    summary["SD_HR"] = metrics_hr_sqi(info[f'{data_type}_clean_rr_systole'], metric='std')
    summary["Min_HR"] = metrics_hr_sqi(info[f'{data_type}_clean_rr_systole'], metric='min')
    summary["Max_HR"] = metrics_hr_sqi(info[f'{data_type}_clean_rr_systole'], metric='max')
...
def sqi_eda(signal_eda, info, sampling_rate=10000):
    ...
    summary = {}
    # Quality indices on overall signal
    summary["Minimal_range"] = minimal_range_sqi(signal_eda['EDA_Clean'], threshold=0.05)
